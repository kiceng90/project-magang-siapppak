                                                ...
                                                <template v-slot:body>
                                                    ...
                                                    >
                                                        ...
                                                        <td class="text-center">
                                                            ...
                                                            >
                                                                <div
                                                                    class="action-datatable-column m-2"
                                                                    @click="
                                                                        edit(
                                                                            context.id
                                                                        )
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-5 mt-5">
                            <label
                                ...
                                "
                                >Judul Subkategori</label
                            >
                            <input
                                class="form-control"
                                type="text"
                                placeholder="Masukkan Judul Subkategori"
                                autocomplete="off"
                                v-model="single.name"
                            />
                            <div
                                ...
                                class="text-danger"
                            >
                                > Judul Subkategori tidak boleh kosong!
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    computed: {
        ...
        filter_list_subkategori_computed() {
            ...
            for (let i = 0; i < this.list_subkategori.length; i++) {
                response.push(this.list_subkategori[i]);
            }
    ...
    methods: {
        ...
        edit(id) {
            ...
            if (index >= 0) {
                ...
                if (
                    this.tableConfig.feeder.data[index].kategori.id &&
                    this.tableConfig.feeder.data[index].kategori.name
                ) {
                    this.single.kategori = {
                        id: this.tableConfig.feeder.data[index].kategori.id,
                        text: this.tableConfig.feeder.data[index].kategori.name,
                    };
                }
        ...
        save() {
            ...
            formData.append(
                "id_kategori",
                this.single.kategori ? this.single.kategori.id : ""
            );

            Api()
                .post("subkategori", formData, {
                    headers: {
        ...
        update() {
            ...
            let formData = new FormData();
            formData.append("name", this.single.name);
            formData.append("id_kategori", this.single.kategori.id);

            console.log(this.single.name);

            // Using PUT to update data on the server
            Api()
                .post(`subkategori/${this.single.id}?_method=PUT`, formData)
                .then((response) => {
                    $("#modal-form").modal("hide");
        ...
        changeStatus(id) {
            this.$ewpLoadingShow();
            Api()
                .put(`subkategori/${id}/switch-status`)
                .then((response) => {
