            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center" colspan="6" rowspan="5">
                        <img
                            v-if="data?.mahasiswa?.sample_mahasiswa?.ttd_url"
                            :src="data.mahasiswa?.sample_mahasiswa.ttd_url"
                            style="height: 150px"
                        />
                    </td>
                </tr>
                ...
                <tr>
                    <td></td>
                    <td class="text-center" colspan="6">{{ name }}</td>
                </tr>
                <tr>
                    <td></td>
...
export default {
    ...
    computed: {
        kabupaten() {
            const balai = this.data?.mahasiswa?.balai_puspaga ?? {};
            return balai.kelurahan?.kecamatan?.kabupaten?.name;
        },
        name() {
            const mhs = this.data?.mahasiswa?.sample_mahasiswa ?? {};
            return mhs.name;
        },
        baseFields() {
            const mhs = this.data?.mahasiswa?.sample_mahasiswa ?? {};
            const balai = this.data?.mahasiswa?.balai_puspaga ?? {};
            return [
                { label: "Nama Lengkap", value: mhs.name },
                { label: "NIK", value: mhs.nik },
                {
                    label: "Status Mahasiswa",
                    value: mhs.pendidikan_terakhir?.name,
                },
                { label: "Jenis Mahasiswa", value: mhs.jenis_mahasiswa?.name },
                { label: "Jurusan", value: mhs.jurusan?.name },
                {
                    label: "Asal Universitas",
                    value: mhs.instansi_pendidikan?.name,
                },
                {
                    label: "Kecamatan Puspaga RW",
                    value: balai.kelurahan?.kecamatan?.name,
                },
                { label: "Kelurahan Puspaga RW", value: balai.kelurahan?.name },
                { label: "Puspaga RW", value: balai.name },
                { label: "Alamat Puspaga RW", value: balai.address },
            ];
        },
        konselingFields() {
            if (this.data) {
                ...
                return [
                    ...
                    },
                    {
                        ...
                        value: data.tipe_permasalahan_string,
                    },
                    {
                        label: "Kategori Kasus",
                        value: data.kategori_kasus?.name || "-",
                    },
        ...
        sosialisasiFields() {
            if (this.data) {
                ...
                return [
                    ...
                    { label: "Narasumber", value: data.name },
                    {
