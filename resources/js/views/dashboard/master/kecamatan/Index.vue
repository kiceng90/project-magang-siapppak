                                        ...
                                        <div class="card-header border-0 pt-5 align-items-center">
                                            <h3 class="card-title align-items-start flex-column">
                                                <span class="card-label fw-bolder text-dark mb-2"
                                                    style="font-size:20px !important;">Master Kecamatan</span>
                                            </h3>
                                        ...
                                        <div class="card-body pt-5">
                                            <div class="row mb-3">
                                                ...
                                                <div class="col-lg-5 pb-5">
                                                    <app-select-single v-model="single.filter.kota"
                                                        :show_button_clear="false" :placeholder="'Pilih Kabupaten/Kota'"
                                                        :options="filter_list_kota_computed" :serverside="true"
                                                        :loading="pageStatus == 'kota-load'" @change-search="getKota"
                                            ...
                                            <app-datatable-serverside :table_config="tableConfig"
                                                ...
                                                <template v-slot:body>
                                                    <tr v-for="(context, index) in tableConfig.feeder.data">
                                                        ...
                                                        <td class="text-left">{{context.wilayah_name}}</td>
                                                        <td class="text-left">{{context.kabupaten_name}}</td>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            {{flag == 'insert' ? 'Tambah Data Kecamatan' : 'Edit Data Kecamatan'}}</h5>
                    ...
                    <div class="modal-body">
                        <div class="mb-5">
                            <label class="form-label fw-bolder fs-6"
                                :class="v$.single.kota.$error ? 'text-danger' : ''">Nama Kabupaten/Kota</label>
                            <app-select-single v-model="single.kota" :placeholder="'Pilih Kabupaten/Kota'"
                                :loading="pageStatus == 'kota-load'" :options="list_kota" :serverside="true"
                                @change-search="getKota">
                            </app-select-single>
                            <div v-if="v$.single.kota.$error" class="text-danger">>Kabupaten/Kota tidak boleh kosong!
                            </div>
                        </div>
                        <div class="mb-5">
                            <label class="form-label fw-bolder fs-6"
                                :class="v$.single.wilayah.$error ? 'text-danger' : ''">Wilayah</label>
                            <app-select-single v-model="single.wilayah" :placeholder="'Pilih Wilayah'"
                                :loading="pageStatus == 'wilayah-load'" :options="list_wilayah" :serverside="true"
                                @change-search="getWilayah">
                            </app-select-single>
                            <div v-if="v$.single.name.$error" class="text-danger">>Wilayah tidak boleh kosong!
                            </div>
                        </div>
                        <div class="mb-5">
                            <label class="form-label fw-bolder fs-6"
                                :class="v$.single.name.$error ? 'text-danger' : ''">Nama Kecamatan</label>
                            <input class="form-control" type="text" placeholder="Masukkan nama kecamatan"
                                autocomplete="off" v-model="single.name" />
                            <div v-if="v$.single.name.$error" class="text-danger">>Nama Kecamatan tidak boleh kosong!
                            </div>
                        </div>
                    </div>
    ...
    export default {
        ...
        computed: {
            filter_list_kota_computed() {
                ...
                for (let i = 0; i < this.list_kota.length; i++) {
                    response.push(this.list_kota[i]);
                }
        ...
        methods: {
            ...
            edit(id) {
                ...
                if (index >= 0) {
                    ...
                    if (this.tableConfig.feeder.data[index].id_kabupaten && this.tableConfig.feeder.data[index]
                        .kabupaten_name) {
                        this.single.kota = {
                            id: this.tableConfig.feeder.data[index].id_kabupaten,
                            text: this.tableConfig.feeder.data[index].kabupaten_name
                        };
                    }

                    if (this.tableConfig.feeder.data[index].id_wilayah && this.tableConfig.feeder.data[index]
                        .wilayah_name) {
                        this.single.wilayah = {
                            id: this.tableConfig.feeder.data[index].id_wilayah,
                            text: this.tableConfig.feeder.data[index].wilayah_name
                        };
                    }
                    $("#modal-form").modal('show');
                }
            ...
            getDataTable() {
                ...
                if (this.single.filter.kota && this.single.filter.kota.id != '0') {
                    Object.assign(params, {
                        id_kabupaten: this.single.filter.kota.id
                    })
                }
            ...
            save() {
                ...
                let formData = {
                    ...
                    id_kabupaten: this.single.kota.id,
                    id_wilayah: this.single.wilayah.id,
                }


                Api().post('m-kecamatan', formData)
                    .then(response => {
                        $("#modal-form").modal('hide');

                        this.$swal({
                            title: "Berhasil!",
                            text: 'Menambahkan data kecamatan',
                            icon: "success",
                        }).then(result => {
            ...
            update() {
                ...
                Api().put(`m-kecamatan/${this.single.id}`, formData)
                    .then(response => {
                        $("#modal-form").modal('hide');

                        this.$swal({
                            title: "Berhasil!",
                            text: 'Memperbarui data kecamatan',
                            icon: "success",
                        }).then(result => {
                            this.getDataTable();
                        });
            ...
            changeStatus(id) {
                this.$ewpLoadingShow();
                Api().put(`m-kecamatan/${id}/switch-status`)
                    .then(response => {
            ...
            getKota(keyword, limit) {

                this.pageStatus = 'kota-load';

                Api().get(`m-kabupaten/lists?limit=${limit}&search=${keyword}`)
                    .then(response => {
