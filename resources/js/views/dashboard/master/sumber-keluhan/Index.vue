                                        ...
                                        <div class="card-header border-0 pt-5 align-items-center">
                                            <h3 class="card-title align-items-start flex-column">
                                                <span class="card-label fw-bolder text-dark mb-2"
                                                    style="font-size:20px !important;">Master Sumber
                                                    Keluhan</span>
                                            </h3>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            {{flag == 'insert' ? 'Tambah Data Sumber Keluhan' : 'Edit Data Sumber Keluhan'}}</h5>
                    ...
                    <div class="modal-body">
                        <div class="mb-5">
                            <label class="form-label fw-bolder fs-6"
                                :class="v$.single.name.$error ? 'text-danger' : ''">Asal Keluhan</label>
                            <input class="form-control" type="text" placeholder="Contoh: Kecamatan" autocomplete="off"
                                v-model="single.name" />
                            <div v-if="v$.single.name.$error" class="text-danger">Asal Keluhan tidak boleh kosong!
                            </div>
                        </div>
                    </div>
    ...
    export default {
        ...
        methods: {
            ...
            save() {
                ...
                Api().post('m-sumber-keluhan', formData)
                    .then(response => {
                        $("#modal-form").modal('hide');

                        this.$swal({
                            title: "Berhasil!",
                            text: 'Menambahkan data sumber keluhan',
                            icon: "success",
                        }).then(result => {
            ...
            update() {
                ...
                Api().put(`m-sumber-keluhan/${this.single.id}`, formData)
                    .then(response => {
                        $("#modal-form").modal('hide');

                        this.$swal({
                            title: "Berhasil!",
                            text: 'Memperbarui data sumber keluhan',
                            icon: "success",
                        }).then(result => {
                            this.getDataTable();
                        });
            ...
            changeStatus(id) {
                this.$ewpLoadingShow();
                Api().put(`m-sumber-keluhan/${id}/switch-status`)
                    .then(response => {
            ...
            exportData(){
                this.$ewpLoadingShow();
                Api().get(`m-sumber-keluhan-export`, {
                    responseType: 'blob',
                }).then(response => {
                    const url = window.URL.createObjectURL(new Blob([response.data]));
                    const link = document.createElement('a');
                    link.href = url;
                    link.setAttribute('download', `Data Master Sumber Keluhan.xlsx`);
