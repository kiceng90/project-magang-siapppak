                                        ...
                                        <div class="card-header border-0 pt-5 align-items-center">
                                            <h3 class="card-title align-items-start flex-column">
                                                <span class="card-label fw-bolder text-dark mb-2"
                                                    style="font-size:20px !important;">Master Instansi Pendidikan</span>
                                            </h3>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            {{flag == 'insert' ? 'Tambah Data Instansi Pendidikan' : 'Edit Data Instansi Pendidikan'}}</h5>
    ...
    export default {
        ...
        methods: {
            ...
            save() {
                ...
                Api().post('m-instansi-pendidikan', formData)
                    .then(response => {
                        $("#modal-form").modal('hide');

                        this.$swal({
                            title: "Berhasil!",
                            text: 'Menambahkan data  Instansi Pendidikan',
                            icon: "success",
                        }).then(result => {
            ...
            update() {
                ...
                Api().put(`m-instansi-pendidikan/${this.single.id}`, formData)
                    .then(response => {
                        $("#modal-form").modal('hide');

                        this.$swal({
                            title: "Berhasil!",
                            text: 'Memperbarui data  Instansi Pendidikan',
                            icon: "success",
                        }).then(result => {
                            this.getDataTable();
                        });
            ...
            changeStatus(id) {
                this.$ewpLoadingShow();
                Api().put(`m-instansi-pendidikan/${id}/switch-status`)
                    .then(response => {
            ...
            exportData(){
                this.$ewpLoadingShow();
                Api().get(`m-instansi-pendidikan/export`, {
                    responseType: 'blob',
                }).then(response => {
                    const url = window.URL.createObjectURL(new Blob([response.data]));
                    const link = document.createElement('a');
                    link.href = url;
                    link.setAttribute('download', `Export Master Instansi Pendidikan.xlsx`);
