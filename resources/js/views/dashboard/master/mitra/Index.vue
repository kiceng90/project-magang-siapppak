<template>
    <dashboard-base-layout>
                        ...
                        <div class="card-header border-0 pt-5 align-items-center">
                            <h3 class="card-title align-items-start flex-column">
                                <span class="card-label fw-bolder text-dark mb-2"
                                    style="font-size:20px !important;">Master Mitra</span>
                            </h3>
                            ...
                            <app-datatable-serverside :table_config="tableConfig"
                                ...
                                <template v-slot:body>
                                    <TableRow
                                        ...
                                        :index="index"
                                        :key="index"
                                        @onEdit="showModalEdit(item.id, item.name, item.id_kategori_mitra, item.address, item.phone)"
                                    />
                                </template>
        ...
        <ModalAdd @onSuccess="onSuccessAddEdit" :kategoriMitraOptions="kategoriMitraOptions"/>
        <ModalEdit :data="modalEditData" @onSuccess="onSuccessAddEdit" :kategoriMitraOptions="kategoriMitraOptions"/>
    </dashboard-base-layout>
</template>
...
export default {
    ...
    data(){
        return  {
            ...
            kategoriMitraOptions: [],
            modalEditData: {
                id: null,
                name: null,
                id_kategori_mitra: null,
                file_photo: null,
                address: '',
                phone: '',
            }
    ...
    methods: {
        ...
        showModalEdit(id, name, id_kategori_mitra, address, phone){
            this.modalEditData = {
                id,
                name,
                id_kategori_mitra,
        ...
        getMKategoryMitra(){
            Api().get(`m-kategori-mitra`).then(response=>{
                const data = response.data.data;
                this.kategoriMitraOptions = data.map(it=>({id: it.id, text: it.name}))
            })
        },
        exportData(){
            this.$ewpLoadingShow();
            Api().get(`mitra-export`, {
                responseType: 'blob',
            }).then(response => {
                const url = window.URL.createObjectURL(new Blob([response.data]));
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', `Export Master Mitra.xlsx`);
