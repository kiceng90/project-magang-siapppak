<template>
    <tr>
        ...
        <td class="text-left">
            <img class="rounded-circle" width="50" height="50" :src="item.file_foto || $assetUrl() + '/siapppak/images/user.png'" alt="Avatar" style="object-fit: cover;">
        </td>
        <td class="text-left">
            <p class="mb-0 fw-bolder">{{ item.name }}</p>
            <p class="mb-0">Alamat : {{ $noNullable(item.address) }}</p>
            <p class="mb-0">Nomor HP : {{ $noNullable(item.phone) }}</p>
        </td>
        <td class="text-left">
            <span class="fw-bolder">{{ item.kategori_mitra?.name || '' }}</span>
        </td>
