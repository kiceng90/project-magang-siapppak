                                        ...
                                        <div class="card-header border-0 pt-5 align-items-center">
                                            <h3 class="card-title align-items-start flex-column">
                                                <span class="card-label fw-bolder text-dark mb-2"
                                                    style="font-size:20px !important;">Master Hubungan Dengan
                                                    Klien</span>
                                            </h3>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            {{flag == 'insert' ? 'Tambah Data Hubungan Dengan Klien' : 'Edit Data Hubungan Dengan Klien'}}
                        </h5>
                    ...
                    <div class="modal-body">
                        <div class="mb-5">
                            <label class="form-label fw-bolder fs-6" :class="v$.single.name.$error ? 'text-danger' : ''">Nama</label>
                            <input class="form-control" type="text" placeholder="Contoh: Suami" autocomplete="off"
                                v-model="single.name" />
    ...
    export default {
        ...
        methods: {
            ...
            save() {
                ...
                Api().post('m-hubungan', formData)
                    .then(response => {
                        $("#modal-form").modal('hide');

                        this.$swal({
                            title: "Berhasil!",
                            text: 'Menambahkan data hubungan dengan klien',
                            icon: "success",
                        }).then(result => {
            ...
            update() {
                ...
                Api().put(`m-hubungan/${this.single.id}`, formData)
                    .then(response => {
                        $("#modal-form").modal('hide');

                        this.$swal({
                            title: "Berhasil!",
                            text: 'Memperbarui data hubungan dengan klien',
                            icon: "success",
                        }).then(result => {
                            this.getDataTable();
                        });
            ...
            changeStatus(id) {
                this.$ewpLoadingShow();
                Api().put(`m-hubungan/${id}/switch-status`)
                    .then(response => {
            ...
            exportData(){
                this.$ewpLoadingShow();
                Api().get(`m-hubungan/export`, {
                    responseType: 'blob',
                }).then(response => {
                    const url = window.URL.createObjectURL(new Blob([response.data]));
                    const link = document.createElement('a');
                    link.href = url;
                    link.setAttribute('download', `Export Master Hubungan Dengan Klien.xlsx`);
