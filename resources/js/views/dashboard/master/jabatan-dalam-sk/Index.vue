                                        ...
                                        <div class="card-header border-0 pt-5 align-items-center">
                                            <h3 class="card-title align-items-start flex-column">
                                                <span class="card-label fw-bolder text-dark mb-2"
                                                    style="font-size:20px !important;">Master Jabatan Dalam SK</span>
                                            </h3>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            {{flag == 'insert' ? 'Tambah Data Jabatan Dalam SK' : 'Edit Data Jabatan Dalam SK'}}</h5>
    ...
    export default {
        ...
        methods: {
            ...
            save() {
                ...
                Api().post('m-kedudukan-dalam-tim', formData)
                    .then(response => {
                        $("#modal-form").modal('hide');

                        this.$swal({
                            title: "Berhasil!",
                            text: 'Menambahkan data Jabatan Dalam SK',
                            icon: "success",
                        }).then(result => {
            ...
            update() {
                ...
                Api().put(`m-kedudukan-dalam-tim/${this.single.id}`, formData)
                    .then(response => {
                        $("#modal-form").modal('hide');

                        this.$swal({
                            title: "Berhasil!",
                            text: 'Memperbarui data Jabatan Dalam SK',
                            icon: "success",
                        }).then(result => {
                            this.getDataTable();
                        });
            ...
            changeStatus(id) {
                this.$ewpLoadingShow();
                Api().put(`m-kedudukan-dalam-tim/${id}/switch-status`)
                    .then(response => {
            ...
            exportData(){
                this.$ewpLoadingShow();
                Api().get(`m-kedudukan-dalam-tim/export`, {
                    responseType: 'blob',
                }).then(response => {
                    const url = window.URL.createObjectURL(new Blob([response.data]));
                    const link = document.createElement('a');
                    link.href = url;
                    link.setAttribute('download', `Export Master Jabatan Dalam SK.xlsx`);
