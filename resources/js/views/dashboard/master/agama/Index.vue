                                        ...
                                        <div class="card-header border-0 pt-5 align-items-center">
                                            <h3 class="card-title align-items-start flex-column">
                                                <span class="card-label fw-bolder text-dark mb-2"
                                                    style="font-size:20px !important;">Master Agama</span>
                                            </h3>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            {{flag == 'insert' ? 'Tambah Data Agama' : 'Edit Data Agama'}}</h5>
                    ...
                    <div class="modal-body">
                        <div class="mb-5">
                            ...
                            <input class="form-control" type="text" placeholder="Contoh: Islam" autocomplete="off"
                                v-model="single.name" />
                            <div v-if="v$.single.name.$error" class="text-danger"> Nama agama tidak boleh kosong!
                            </div>
                        </div>
                    </div>
    ...
    export default {
        ...
        methods: {
            ...
            save() {
                ...
                Api().post('m-agama', formData)
                    .then(response => {
                        $("#modal-form").modal('hide');

                        this.$swal({
                            title: "Berhasil!",
                            text: 'Menambahkan data agama',
                            icon: "success",
                        }).then(result => {
            ...
            update() {
                ...
                Api().put(`m-agama/${this.single.id}`, formData)
                    .then(response => {
                        $("#modal-form").modal('hide');

                        this.$swal({
                            title: "Berhasil!",
                            text: 'Memperbarui data agama',
                            icon: "success",
                        }).then(result => {
                            this.getDataTable();
                        });
            ...
            changeStatus(id) {
                this.$ewpLoadingShow();
                Api().put(`m-agama/${id}/switch-status`)
                    .then(response => {
            ...
            exportData(){
                this.$ewpLoadingShow();
                Api().get(`m-agama/export`, {
                    responseType: 'blob',
                }).then(response => {
                    const url = window.URL.createObjectURL(new Blob([response.data]));
                    const link = document.createElement('a');
                    link.href = url;
                    link.setAttribute('download', `Export Master Agama.xlsx`);
