<template>
    <dashboard-base-layout>
                        ...
                        <div class="card-header border-0 pt-5 align-items-center">
                            <h3 class="card-title align-items-start flex-column">
                                <span class="card-label fw-bolder text-dark mb-2"
                                    style="font-size:20px !important;">Master Jadwal Konseling</span>
                            </h3>
                            ...
                            <button type="button" class="btn btn-sm bg-primary-custom ms-2"
                                @click="showModalAdd('Konselor')">
                                <span class="text-white">Tambah Jadwal Konselor</span>
                            </button>
                            <button type="button" class="btn btn-sm bg-primary-custom ms-2"
                                @click="showModalAdd('Psikolog')">
                                <span class="text-white">Tambah Jadwal Psikolog</span>
                            </button>
                        </div>
                        ...
                        <div class="card-body pt-5">
                            ...
                            >
                                <template v-slot:body>
                                    ...
                                    >
                                        <template #menu>
                                            <li v-for="day in item.jadwal">
                                                <a class="dropdown-item py-3" href="javascript:void(0);" @click="showModalEdit(item.name, item.consultant_type, day)">Edit Jadwal Hari {{ day.day_string }}</a>
                                            </li>
                                        </template>
                                    </TableRow>
                                </template>
        ...
        <ModalAdd @onSuccess="onSuccessAddEdit" :kategoriKonselingOptions="kategoriKonselingOptions" :type="modalAddData.type"/>
        <ModalEdit @onSuccess="onSuccessAddEdit" :kategoriKonselingOptions="kategoriKonselingOptions" :data="modalEditData"/>
    </dashboard-base-layout>
</template>
...
export default {
    ...
    methods: {
        ...
        showModalEdit(name, type, jadwal){
            this.modalEditData = {
                konsultan_type: type,
                konsultan_name: name,
                ...jadwal,
        ...
        getKategoriKonseling(){
            Api().get(`m-kategori-konseling`)
                .then(response => {
                    this.kategoriKonselingOptions = response.data.data.map(it=>({id: it.id, text: it.name}));
                })
                .catch(error => {
                    this.$axiosHandleError(error);
                }).then(() => {
        ...
        exportData(){
                this.$ewpLoadingShow();
                Api().get(`m-jadwal-konseling-detail-export`, {
                    responseType: 'blob',
                }).then(response => {
                    const url = window.URL.createObjectURL(new Blob([response.data]));
                    const link = document.createElement('a');
                    link.href = url;
                    link.setAttribute('download', `Export Master Jadwal Konseling.xlsx`);
