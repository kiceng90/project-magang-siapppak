<template>
    <tr>
        ...
        <td class="text-left">
            {{ item.question }}<br/><span class="text-muted">{{ item.key }}</span>
        </td>
        <td class="text-left">
            {{ item.sub_kategori?.kategori?.name }}
        </td>
        <td class="text-left">
            {{ item.sub_kategori?.name }}
        </td>
        <td class="text-left">
            {{ item.input_type_string }}
        </td>
        <td class="text-left">
            {{ item.order }}
        </td>
...
export default {
    ...
    methods:{
        changeStatus() {
            this.$ewpLoadingShow();
            Api().put(`m-kuesioner-laporan-monev/${this.item.id}/switch-status`)
                .then(response => {
