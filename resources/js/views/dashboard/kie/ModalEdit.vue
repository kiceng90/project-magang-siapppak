<template>
    <dashboard-modal title="Edit Data" id="modal-edit" dialog-class="modal-lg">
        ...
        <input v-if="type == this.enumType.GAMBAR.value" type="file" class="form-control" v-on:change="onChangeImage" accept="image/*" ref="file_image">
        <input v-if="type == this.enumType.PDF.value" type="file" class="form-control" v-on:change="onChangePdf" accept="application/pdf" ref="file_pdf">
        <input v-if="type == this.enumType.VIDEO.value" type="text" class="form-control mb-5" v-model="url_video_youtube" placeholder="Url video">
    ...
    watch: {
        data(n, p){
            if(n!==p){
                this.type = n.type;
                this.title = n.title;
                this.description = this.$refs.editor.setHTML(n.description);
                this.url_video_youtube = n.url_video_youtube;
                this.previewThumbnailSrc = n.file_thumbnail;
            }
        }
    },
    methods: {
        ...
        onSubmit(){
            ...
            if(this.file_thumbnail) formData.append('file_thumbnail', this.file_thumbnail);
            ...
            Api().post(`kie/${this.data.id}`, formData)
                .then(response => {
