...
<table class="table" cellspacing="0" style="font-family:Arial;">
    <thead>
        <tr>
            ...
            <th colspan="3" style="background:#9bbb59;text-align:center;">PELAYANAN {{$i + 1}}</th>
            @endfor            
        </tr>
        <tr>
    ...
    @foreach($data ?? [] as $key => $context)
        <tr>
            ...
            <td style="text-align:center;">
                {{$context->pelayanan && count($context->pelayanan) > $i && $context->pelayanan[$i] ? $context->pelayanan[$i]->kebutuhan : ''}}</td>
            <td style="text-align:center;">
                {{$context->pelayanan && count($context->pelayanan) > $i && $context->pelayanan[$i] ? $context->pelayanan[$i]->opd : ''}}</td>            
            <td style="text-align:center;">
