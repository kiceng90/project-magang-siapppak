    ...
    <div class="box-container-print">
        ...
        <table class="table-print">
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if ($identifikasiTerbaru)
                            {{ $identifikasiTerbaru->disposisi_pimpinan
                                ? 'Warga Surabaya (Berikan layanan puspaga sesuai kebutuhan klien)'
                                : 'Warga Non Surabaya (Berikan layanan puspaga sesuai ketentuan)' }}
                        @else
                            {{ $bukuTamu->disposisi ? 'Segera Tindaklanjuti' : 'Segera Tindaklanjuti Dengan ketentuan' }}
                        @endif
                    </td>
                </tr>
            </tbody>
        ...
        @if ($identifikasiTerbaru)
            <table class="table-print">
                <tbody>
                    <tr>
                        <td colspan="3"><b>IDENTIFIKASI KEBUTUHAN</b></td>
                    </tr>
                    <tr>
                        <td width="30%">HARI/TANGGAL</td>
                        <td width="4%" class="text-center">:</td>
                        <td>{{ app()->make('App\Http\Controllers\API\BukuTamuController')->getHari($identifikasiTerbaru->created_at) }},
                            {{ app()->make('App\Http\Controllers\API\BukuTamuController')->getTanggal($identifikasiTerbaru->created_at) }}
                        </td>
                    </tr>
                    <tr>
                        <td width="30%">PEMBERIAN DUKUNGAN PSIKOLOGIS AWAL</td>
                        <td width="4%" class="text-center">:</td>
                        <td>{{ $identifikasiTerbaru->dpa ? 'Terdapat narasi singkat DPA yang dilakukan.' : 'Tidak ada isian' }}
                        </td>
                    </tr>
                    @if ($identifikasiTerbaru->dpa == 1)
                        <tr>
                            <td width="30%">NARASI</td>
                            <td width="4%" class="text-center">:</td>
                            <td>{{ $identifikasiTerbaru->narasi }}
                            </td>
                        </tr>
                    @endif
                    <tr>
                        <td width="30%">HASIL ASSESMENT</td>
                        <td width="4%" class="text-center">:</td>
                        <td>{{ $identifikasiTerbaru->hasil_assesment }}
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <table class="table-print">
            <tbody>
                <tr>
                    <td colspan="3"><b>ALAMAT PENGUNJUNG</b></td>
                </tr>
                <tr>
